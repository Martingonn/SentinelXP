...
void PrintUsage(SIZE_T& winMem, SIZE_T& otherMem) {
    ...
    SIZE_T totalMem = winMem + otherMem;
...
int main() {
    ...
    double sumWinMB = 0.0, sumOtherMB = 0.0, sumTotalMB = 0.0;
    int sampleCount = 0;
    ...
    double avgTotalMB = sampleCount ? (sumTotalMB / sampleCount) : 0.0;
    ...
    if (f) {
        ...
        fprintf(f, "Average total RAM used:               %.2f MB\n", avgTotalMB);
        fprintf(f, "Samples: %d\n\n", sampleCount);
        fclose(f);
    } else {
